(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if ("IntersectionObserver" in window) {
      ...
      document
        .querySelectorAll(".counter, .stat-num")
    ...
    (function initTimer() {
      ...
      if (daysEl && hoursEl && minsEl && secsEl) {
        const newYear = new Date(
          `Jan 01 ${new Date().getFullYear() + 1} 00:00:00`,
        );
        function update() {
          const now = new Date();
          ...
          hoursEl.innerText = String(Math.floor(diff / 3600) % 24).padStart(
            2,
            "0",
          );
          minsEl.innerText = String(Math.floor(diff / 60) % 60).padStart(
            2,
            "0",
          );
      ...
      links.forEach((link) => {
        ...
        if (linkFile === filename) {
          ...
          const parent = link.closest(
            ".nav-item, .responsive-menu-list, .gm-item",
          );
          if (parent) parent.classList.add("active");

          const dropdown = link.closest(
            ".dropdown-menu, .gm-submenu, .responsive-menu-items",
          );
          if (dropdown) {
            const btn = dropdown.parentElement.querySelector(
              ".dropdown-toggle, .gm-dropbtn, a",
            );
      ...
      if (toggle) {
        ...
        toggle.addEventListener("click", function (e) {
          ...
          const siblings = parent
            ? parent.querySelectorAll(":scope > .responsive-menu-list")
            : [];
  ...
  mobileLinks.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      if (
        ...
        link.nextElementSibling?.classList.contains("responsive-menu-items")
      ) {
        return;
      }
